<?php
    ...
    global $username,$name,$idTopic,$title,$image,$image_path,$idQuest,$quest,$answer,$score,$id_edit,$url_img,$privacy;



    if(isset($_GET["idTopic"])){
        $_SESSION['idTopic'] = $_GET["idTopic"];
        $tmpTopic = getTopic($_GET["idTopic"]);

        // echo $privacy;
        // exit();
        $_SESSION['title'] = $tmpTopic['titleTopic'];
        $_SESSION['status'] = $tmpTopic['status'];
        $privacy = $_SESSION['status'];
    }
    if(isset($_GET["edit-topic"])){
        $editTopic = trim($_GET['edit-topic']);
        $newTitle = trim($_POST['edit-title']);

        $newprivacy = trim($_POST['privacy-input']);
        if($newprivacy != $privacy){
            updateStatusTopic($editTopic,$newprivacy);
            $_SESSION['status'] = $newprivacy;
        }
        updateTitleTopic($editTopic,$newTitle);
        updateTimeTopic($editTopic);
        $_SESSION['title'] = $newTitle;
        header("location: update-detail.php");
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"&& !isset($_GET["edit-topic"])  &&$_POST["quest"]!="") {
...
<body>
    ...
    <div class="heading">
        <a id="back" href="../dashboard.php"><i class="ri-arrow-go-back-line"></i>Quay lại trang chủ</a>
        <div class="loader">
            <span>TOPIC</span>
            <span>TOPIC</span>
        </div>
        ...
        <form id="heading-form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . "?edit-topic=" .$idTopic; ?>" method="POST">
            <input type="text"  name="edit-title" value="<?php echo $title; ?>">
            <!-- <p id="stust"></p> -->
            <input id="edit-topic" type="submit" value="Lưu">
            <div class="privacy-topic">
                <label for="">Quyền chia sẻ:</label>
                <label class="switch">
                    <input type="checkbox" name="privacy-input" id="thecheckbox" class="input__check" value=" <?php echo $privacy ?>" onclick="changeButton()" >
                    <span class="slider">
                        <p id="private" style="display:none" ><i class='ri-git-repository-private-fill'></i></p>
                        <p id="public" ><i class='ri-share-line'></i></p>
                    </span>
                </label>
                <script>
                    ...
                    function changeButton() {
                        ...
                        if (!checkboxvar.checked) {
                            checkboxvar.checked = "uncheck";
                            checkboxvar.value ="1";
                            public.style.display = "inline";
                            private.style.display = "none";
                        }
                        else {
                            checkboxvar.checked = "checked";
                            checkboxvar.value = "0";
                            public.style.display = "none";
                            private.style.display = "inline";
                        }
                        document.getElementById("heading-form").submit();
                    }
                </script>
            </div>
        </form>
                        ...
                        <div class="preview">
                            <div class="card">
                                <div class="first-content">
                                    <img id="output" src="<?php if(isset($_SESSION['editing'])) { echo $image_path;} else {echo "../image/input-photo.png";} ?>" >
                                </div>
                                    ...
                                    while ($row = mysqli_fetch_array($result)) {
                                        ...
                                        echo "<tr>";
                                            ...
                                            echo "<td style = 'width: 20%'>";
                                                echo "<a href='update-detail.php?act=edit&id=".$row['idQuest']."' title ='Edit' data-toggle = 'tooltip'><i class='ri-pencil-fill'></i></a>";
                                                echo "<i style=' color: transparent' class='ri-space'></i>";
                                                echo "<a href='update-detail.php?act=delete&id=".$row['idQuest']."' title = 'Delete' data-toggle = 'tooltip'><i class='ri-delete-back-2-line'></i></a> </td>";
                                        echo "</tr>";
